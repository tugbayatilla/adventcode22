...
public class AdventDay4UnitTests
{
    ...
    [Fact(Skip = $"new theory test is cover this test. {nameof(_008_check_overlap_pairs_fully_with_given_pair_definitions)}")]
    public void _007_overlap_pairs_returns_true()
    {
        Assert.True(_adventDay4.IsOverlapFully("1-1, 1-1"));
    }
    ...
    public void _008_check_overlap_pairs_fully_with_given_pair_definitions(string pairDefinition, bool expected)
    {
        Assert.Equal(expected, _adventDay4.IsOverlapFully(pairDefinition));
    }
    ...
    public void _008_check_overlap_pairs_partially_with_given_pair_definitions(string pairDefinition, bool expected)
    {
        Assert.Equal(expected, _adventDay4.IsOverlapPartially(pairDefinition));
    }
}